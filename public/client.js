...
socket.on('ready', async (socketId, name) => {
    ...
    peerConnection.setLocalDescription(offer)
        .then(() => socket.emit('offer', socketId, peerConnection.localDescription, clientId));
...
async function start() {
    ...
    localVideo.srcObject = localStream = await navigator.mediaDevices.getUserMedia(mediaStreamConfig);
    localVideo.play();
...
async function demonstrateScreen() {
    ...
    if (localVideo.srcObject === localStream) {
        currentStream = await navigator.mediaDevices.getDisplayMedia(mediaStreamConfig);
    } else {
